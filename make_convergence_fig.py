...
_commas = FuncFormatter(lambda v, _: f'{int(round(v)):,}' if abs(v) >= 10000
...
for ax, (key, label) in zip(axes, panels):
    ...
    ax.xaxis.set_major_formatter(_commas)
    ax.grid(alpha=0.3)
...
for out in ('submission/figures', 'figures'):
    ...
    plt.savefig(f'{out}/training_convergence.pdf', bbox_inches='tight')
    plt.savefig(f'{out}/training_convergence.png', bbox_inches='tight', dpi=300)
print('wrote training_convergence.pdf to submission/figures and figures')
